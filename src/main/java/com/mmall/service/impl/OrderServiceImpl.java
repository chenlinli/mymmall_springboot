...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        if(CollectionUtils.isEmpty(orderItemList)){
            return ServerResponse.createByErrorMessage("购物车空");
        }
        ...
        //生成成功，减少产品库存
        reduceProductStack(orderItemList);
     ...
     * @param orderItemList
     */
    private void reduceProductStack(List<OrderItem> orderItemList) {
        for(OrderItem orderItem:orderItemList){
       ...
       List<OrderItem> orderItems = Lists.newArrayList();
        ...
        for (Cart cartItem:cartList) {
            ...
            if(Const.ProductStatusEnum.ON_SALE.getCode()!=product.getStatus()){
                return ServerResponse.createByErrorMessage("产品不是售卖状态");
            }
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        if(order.getStatus()!= Const.OrderStatusEnum.NO_PAY.getCode()){
           return ServerResponse.createByErrorMessage("已经付款无法取消");
        }
        Order updateOrder = new Order();
        updateOrder.setId(order.getId());
        updateOrder.setStatus(Const.OrderStatusEnum.CANCLE.getCode());
    ...
    public ServerResponse getOrderCartProduct(Integer userId) {
        ...
        orderProductVo.setImagHost(ftpConfig.getServerHttpPrefix());
    ...
    public ServerResponse getOrderDetail(Integer userId, long orderNo) {
        Order order = orderMapper.selectByUserIdOrderNo(userId,orderNo);
        if(order==null){
            ServerResponse.createByErrorMessage("该用户次订单不存在");
        }
    ...
    public ServerResponse<PageInfo> list(Integer userId, int pageSize, int pageNum) {
        ...
        List<OrderVo> orderVoList =  assembelOrderVoList(orderList,userId);
        PageInfo pageInfo = new PageInfo(orderList);
        pageInfo.setList(orderVoList);
        return ServerResponse.createBySuccess(pageInfo);
    }

    private List<OrderVo> assembelOrderVoList(List<Order> orderList, Integer userId) {
    ...
    @Override
    public ServerResponse<PageInfo> manageList(int pageSize, int pageNum) {
        PageHelper.startPage(pageNum,pageSize);
        List<Order> orderList = orderMapper.selectAll();
        List<OrderVo> orderVoList =  assembelOrderVoList(orderList,null);
        PageInfo pageInfo = new PageInfo(orderList);
        pageInfo.setList(orderVoList);
        return ServerResponse.createBySuccess(pageInfo);
    }
    ...
    public ServerResponse pay(Integer userId, String path, Long orderNo){
        ...
        reslutMap.put("orderNo",String.valueOf(orderNo));
        ...
        String subject = new StringBuilder().append("happy mall 扫码支付，订单号：").append(outTradeNo).toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                String qrUrl = ftpConfig.getServerHttpPrefix()+targetFile.getName();
    ...
    public ServerResponse aliCallback(Map<String,String> params){
        ...
        //补充
        if(!order.getPayment().equals(new BigDecimal(totalAmount))){
            return ServerResponse.createByErrorMessage("支付回调金额不一致");
        }
